<template>
    <div class="p-2 md:p-6">
        <h1 class="text-2xl font-bold mb-4">{{ isEditing ? 'Editar Ubicación' : 'Crear Ubicación' }}</h1>
...
onMounted(async () => {
    try {
        if (isEditing.value) {
            const response = await axios.get(`/api/admin/inventory/locations/${route.params.id}`);
...
const handleSubmit = async () => {
    try {
        if (isEditing.value) {
            await axios.put(`/api/admin/inventory/locations/${route.params.id}`, form.value);
        } else {
            await axios.post('/api/admin/inventory/locations', form.value);
        }
