<template>
    <div class="p-6">
        ...
        <DataTable :headers="headers" :items="users" :pagination="pagination" @update:options="fetchUsers">
            <template #row="{ item }">
                <td class="py-3 px-6 text-left whitespace-nowrap">{{ item.id }}</td>
                <td class="py-3 px-6 text-left">{{ item.nombre }} {{ item.apellido }}</td>
                <td class="py-3 px-6 text-left">{{ item.email }}</td>
                <td class="py-3 px-6 text-left">{{ item.role?.nombre_corto }}</td>
...
onMounted(async () => {
    await fetchUsers(currentOptions.value);
});

const fetchUsers = async (options) => {
    currentOptions.value = { ...currentOptions.value, ...options };
    try {
        const response = await axios.get('/api/admin/users', { params: currentOptions.value });
...
const deleteUser = async (id) => {
    ...
    try {
        ...
        fetchUsers(currentOptions.value);
    } catch (e) {
        console.error(e);
